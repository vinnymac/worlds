...
- 64e46ba: Implement `events.listByCorrelationId` instead of silently returning nothing.
  ...
  unscoped throw.
- 10658da: Honour `resolveData` on the `events.create` return path too.
...
- b4238f3: Stop `{ timeoutSeconds }` suspensions from consuming Cloudflare's retry budget,
