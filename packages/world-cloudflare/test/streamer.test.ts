...
describe('Streamer (StreamDO RPC integration)', () => {
  ...
  describe('streams.writeMulti()', () => {
    it('should write chunks in order', async () => {
      await streamer.streams.writeMulti?.('wrun_123', 'multi-stream', [
        'one',
        new Uint8Array([2]),
        'three',
      ]);
      await streamer.streams.close('wrun_123', 'multi-stream');

      const chunks = await readAll(await streamer.streams.get('wrun_123', 'multi-stream'));
      expect(chunks).toHaveLength(3);
      expect(new TextDecoder().decode(chunks[0])).toBe('one');
      expect(Array.from(chunks[1])).toEqual([2]);
      expect(new TextDecoder().decode(chunks[2])).toBe('three');
    });
  });
