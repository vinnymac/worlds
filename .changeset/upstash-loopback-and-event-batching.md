...
N round trips. They now issue a single `MGET`, which is the dominant per-replay
saving under load.
...
otherwise fire unboundedly many concurrent requests at the harness server.
`'qstash'` mode is untouched.
